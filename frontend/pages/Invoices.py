...
st.set_page_config(page_title="Invoice Management", layout="wide")
...
st.caption("Manage customer invoices, track billing statuses, and process payments.")
...
with st.expander("🔍 Search & Filter Options", expanded=True):
    ...
    with col2:
        ...
        sort_by = st.selectbox(
            "Sort By",
            ["created_at", "due_date", "total_amount", "invoice_number"],
            key="inv_sort_by"
        )
...
pending_count = sum(1 for i in invoices if i.get("status") == "pending")
...
if not invoices:
    ...
    for inv in invoices:
        ...
        customer_email = inv.get("customer_email") or f"User #{inv['user_id']}"
        customer_uname = inv.get("customer_username") or ""
        ...
        with st.container(border=True):
            head_col1, head_col2, head_col3, head_col4 = st.columns([2, 3, 2, 2])
            ...
            with head_col2:
                st.write(f"**Customer:** {customer_uname} (`{customer_email}`)")
                created_str = inv.get("created_at", "")[:10] if inv.get("created_at") else "N/A"
            ...
            with st.expander("📄 View Full Invoice Details & Actions"):
                # Fetch full details
                ...
                if detail_resp.status_code == 200:
                    ...
                    with d_col1:
                        ...
                        st.write(f"- **User ID:** {cust.get('id', 'N/A')}")
                        st.write(f"- **Username:** {cust.get('username', 'N/A')}")
                        st.write(f"- **Email:** {cust.get('email', 'N/A')}")

                        st.markdown("#### 📅 Billing Period & Dates")
                        p_start = detail.get("billing_period_start")
                        p_end = detail.get("billing_period_end")
                        due = detail.get("due_date")
                        st.write(f"- **Period Start:** {p_start[:10] if p_start else 'N/A'}")
                        st.write(f"- **Period End:** {p_end[:10] if p_end else 'N/A'}")
                        st.write(f"- **Due Date:** {due[:10] if due else 'N/A'}")

                    with d_col2:
                        st.markdown("#### 📦 Subscription Information")
                        sub = detail.get("subscription")
                        if sub:
                            st.write(f"- **Subscription ID:** #{sub.get('id')}")
                            st.write(f"- **Plan ID:** #{sub.get('plan_id')}")
                            st.write(f"- **Status:** `{sub.get('status')}`")
                        else:
                            st.write("_No active subscription linked._")
                    ...
                    # Action buttons
                    st.divider()
                    act_col1, act_col2 = st.columns([2, 5])

                    with act_col1:
                        if status in ["pending", "overdue"] and pay_status != "paid":
                            if st.button("💳 Process Payment", key=f"pay_btn_{inv_id}", use_container_width=True):
                                ...
                                if pay_resp.status_code == 200:
                                    res = pay_resp.json()
                                    st.success(f"✅ {res.get('message', 'Payment processed successfully!')}")
                                    st.rerun()
                                else:
                                    try:
                                        err_detail = pay_resp.json().get("detail", "Payment failed.")
                                    except Exception:
                                        err_detail = "Payment processing error."
                                    st.error(f"❌ {err_detail}")
                        elif status == "paid":
                            st.success("✅ Invoice is fully paid.")
                        else:
                            st.info(f"Status: {status}")

                else:
                    st.error("Failed to load detailed invoice information.")
...
with p_col2:
    st.write(f"Page **{current_page}** of **{max(1, total_pages)}** (Total: {total_count} invoices)")
