...
def test_full_billing_flow_dunning_to_cancellation(db_session, monkeypatch):
    """
    trial -> active -> payment fails -> past_due -> retries
    exhausted -> cancelled
    """

    monkeypatch.setattr("app.workers.tasks.random.random", lambda: 1.0)
    subscription, invoice, payment = _seed_billing_chain(db_session)
    ...
    process_payment_retries(db_session)
