...
export async function getAdminUsers(): Promise<{ total: number; items: AdminUser[] }> {
  ...
  const result = await parseResponse(response);
  if (!response.ok) throw new Error(getErrorMessage(result, "Failed to load users"));
  return result;
}

export async function updateAdminUser(id: number, data: Partial<Pick<AdminUser, "role" | "is_active" | "first_name" | "last_name" | "phone">>) {
  ...
  const result = await parseResponse(response);
  if (!response.ok) throw new Error(getErrorMessage(result, "Failed to update user"));
...
export async function deleteAdminUser(id: number) {
  ...
  if (!response.ok) {
    const result = await parseResponse(response);
    throw new Error(getErrorMessage(result, "Failed to delete user"));
  }
}
